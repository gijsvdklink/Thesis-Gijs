"""
v3_dalmau_dense — dalmau env, 2-3 aircraft in a high-density circular sector.

Identical reward / observation / action design to v3_dalmau; only the airspace
geometry differs (see CONFIG): few aircraft, small area per aircraft (high
density), near-circular sector. Experiment: does the dalmau policy learn
conflict resolution in a dense circular setting with very little traffic?
...
CONFIG = {
    ...
    'altitude':              350,
    'center_ll':             (52.3, 5.3),
    'n_aircraft':            lambda: random.randint(2, 3),
    'density_km2':           lambda: random.uniform(4_000.0, 8_000.0),  # km^2 PER aircraft:
                                                                         # area = n * density_km2.
                                                                         # smaller -> higher density.
                                                                         # 4-8k/ac -> ~8-24k km^2 total
                                                                         # (~55-95 NM across): frequent
                                                                         # conflicts, matches test_env.
    'sep_nm':                5.0,
    'buffer_nm':             10.0,
    'dest_dist_factor':      2.0,
    # Polygon
    'n_vertices':            lambda: 80,        # ~80 needed to reach circ 0.99 (a 7-gon maxes ~0.91)
    'min_circularity':       0.99,              # near-circular sector
