...
aircraft (ownship), extended to the 4 nearest/most-urgent intruders. All states
are normalised in-env to [-1,1]/[0,1] by fixed physical ranges (the ACAS Xu
inputs scaled by their known spans), so NO VecNormalize obs scaling is needed:
  ownship (shared, 4):
    [0]  sin(dpsi_dest)   heading error (cmd vs bearing-to-destination), sin/cos
    [1]  cos(dpsi_dest)   encoded to avoid the +-180 deg wrap discontinuity
    [2]  v_own            ownship speed / nominal cruise (V_NOM)         ~[0, 1]
    [3]  turn_progress    (cmd_hdg - actual_hdg) / 180, in [-1, 1]: outstanding
                          turn (0 = aircraft has settled onto the commanded heading)
  per intruder (4 slots x 5), urgency-desc then nearest-asc:
    rho    distance to intruder / D_WARN (warning horizon)              [0, 1]
    theta  angle to intruder relative to ownship heading / pi (0=ahead) [-1, 1]
    psi    intruder heading relative to ownship heading / 180           [-1, 1]
...
OBS_DIM = 4 + N_NBR * 5       # 4 ownship (sin/cos dest, v_own, turn_progress) + 4 intruders x 5 (rho, theta, psi, v_int, tau) = 24
...
OBS_OWNSHIP_LABELS  = ['sin Dpsi', 'cos Dpsi', 'v_own', 'turn_prog']
...
class AirspaceEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # re-aim every fly-direct aircraft at its destination before propagating
    ...
    def _get_observation(self):
        ...
        if cs is None or bs.traf.id2idx(cs) < 0:
            # no controllable aircraft: on-route (dpsi=0), nominal speed, slots empty
        ...
        dest_bearing, _ = geo.kwikqdrdist(
            ...
            *[float(v) for v in self._destination_ll[cs]])
        hdg_err = wrap_to_180(dest_bearing - cmd_hdg)
        turn_progress = wrap_to_180(cmd_hdg - own_hdg) / 180.0   # [-1,1]; 0 = turn complete
        ...
        obs = [math.sin(math.radians(hdg_err)),       # sin(dpsi_dest)
               ...
               turn_progress]                         # outstanding turn (cmd vs actual heading)
        ...
        for other_cs in self._active_callsigns:
            ...
            # ACAS Xu states, normalised by fixed physical ranges:
            rho   = min(1.0, dist_nm / D_WARN)                         # [0] distance / warning horizon
            theta = math.atan2(ego_lat, ego_fwd) / math.pi            # [1] bearing to intruder / pi (0 = ahead)
            psi   = wrap_to_180(int_hdg - own_hdg) / 180.0            # [2] intruder hdg rel. ownship / 180
