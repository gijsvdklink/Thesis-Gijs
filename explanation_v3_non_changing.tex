...
\State $r \leftarrow r_\text{safe} + r_\text{nav} - w_\text{work}\cdot c$
\State \Return $\mathbf{o}(\text{ownship}),\; r,\;
               \text{terminated}=(\text{ownship exited}),\;
...
The reward formula is identical to v3\_improved, evaluated for the ownship only.
Traffic-vs-traffic conflicts carry no penalty.
\begin{equation}
    r_t = r_\text{safe} + r_\text{nav} - w_\text{work}\cdot c.
\end{equation}

\paragraph{Safety term.}
Every traffic aircraft contributes an exponential proximity cost, normalised by
the number of traffic aircraft $N_\text{int}$ to keep the signal scale-invariant:
\begin{equation}
    r_\text{safe}
...
to traffic aircraft $k$ within $T_\text{look} = 900$\,s.
Each term lies in $(0,1]$, so $r_\text{safe} \in [-w_\text{safe}, 0)$ regardless
of how many traffic aircraft are present.
...
This term is positive when the ownship is heading toward its destination at
cruise speed, negative when heading away, and is always nonzero.
The net reward $r_\text{safe} + r_\text{nav}$ is positive only when the ownship
is simultaneously on track and well separated from all traffic
($d_\text{CPA} \gtrsim 8$\,NM on average when perfectly aligned).

\paragraph{Episode termination.}
The episode terminates ({\tt terminated = True}) as soon as the ownship exits
the sector, in addition to the standard step-count truncation.
No sparse exit reward is applied: the navigation term provides a continuous
incentive for efficient transit, and a terminal reward would introduce credit
...
Episode termination  & Step-count truncation only     & Ownship exit or truncation \\
Observation          & From most urgent aircraft      & Always from ownship \\
Action space         & Discrete(9)                    & Discrete(9) \\
Observation dim      & 21                             & 21 \\
Reward formula       & $r_\text{safe}+r_\text{nav}-w_\text{work}c$ & identical \\
\bottomrule
\end{tabular}
\end{table}
