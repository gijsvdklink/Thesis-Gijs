...
$3$ & $p_\text{turn}$ & $\psi_\text{cmd}-\psi_\text{act}$: turn-execution lag (rad) & $[-\pi,\pi]$\\
$4$ & $S_\text{now}$ & conflict score on current heading & $[0,1]$\\
$5$ & $S_\text{ret}$ & conflict score on route heading, infinite horizon & $[0,1]$\\
\multicolumn{4}{l}{\emph{Conflicting aircraft and nearest intruders $j$}}\\
$+0$ & $\rho$ & $d_{fj}/d_\text{warn}$: distance to intruder, normalised & $[0,1]$\\
$+1$ & $\theta$ & angle to intruder relative to ownship heading (rad) & $[-\pi,\pi]$\\
$+2$ & $\psi_j$ & heading angle of intruder relative to ownship heading (rad) & $[-\pi,\pi]$\\
