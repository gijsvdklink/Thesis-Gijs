...
PPO_KWARGS = dict(
    learning_rate = 3e-4,
    n_steps       = 256,    # rollout = n_steps*N_ENVS = 24,576 -- 4x more frequent
                             # updates and reward/tensorboard dumps than 1024
    batch_size    = 2048,   # was 64: with N_ENVS=96 a rollout is large (24,576
                             # steps); batch_size=64 meant 384 tiny minibatches
                             # x 10 epochs = 3,840 optimiser steps per rollout,
                             # mostly Python/Adam overhead. 2048 -> 12 minibatches
                             # x 10 epochs = 120 steps: far less wall-clock spent
                             # per rollout, and lower-variance gradients from
                             # bigger batches tends to help PPO stability too.
    n_epochs      = 10,
    gamma         = 0.99,
    gae_lambda    = 0.95,
    clip_range    = 0.2,
...
def train(seed, t_warn=None):
    t_warn_tag = f"_twarn{int(t_warn)}s" if t_warn is not None else ""
    run_name = f"v3_8act_obs23_seed{seed}{t_warn_tag}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
