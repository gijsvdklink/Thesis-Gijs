...
Reward is purely negative (no positive components):
  ...
  -w_drift    x ((1-cos(psi_dest-psi_cmd))/2)^2              medium: commanded heading deviation from route,
                                                              squared -- punishes extreme angles much harder
                                                              than mild ones
...
Action space (Discrete 8) -- ATC-styled hybrid, consecutive:
  ...
  7  FLY DIRECT       psi_cmd = psi_dest         (free)
...
CONFIG = {
    ...
    'w_drift':               1.50,            # accumulates during hold; squared -> mild for small
                                               # deviations, sharply heavier for extreme ones
    ...
    def _compute_reward(self, acting_cs, action_idx):
        ...
        if acting_cs and acting_cs in self._destination_ll:
            ...
            if idx >= 0:
                ...
                # Squared (not linear) so mild corrective deviations stay cheap while
                # extreme/near-reversal angles are punished much more steeply.
                drift_frac = (1.0 - math.cos(math.radians(hdg_err))) / 2.0
                r_drift    = -CONFIG['w_drift'] * drift_frac ** 2
