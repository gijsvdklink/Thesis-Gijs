...
The \texttt{v3\_dalmau} environment simulates a single upper-airspace sector
in which a reinforcement learning agent acts as an air traffic controller,
issuing heading instructions to resolve conflicts between en-route aircraft.
Two design choices distinguish it from earlier versions.

First, the action space combines \textit{ATC-styled turn instructions} with
two free persistence actions.
Turn instructions set the commanded heading as a one-shot offset from the
current destination bearing, recomputed at the time of execution, so the
deviation is always expressed relative to the live direct-to-destination
track.
A \textit{hold} action re-issues the commanded heading unchanged, allowing
the aircraft to execute a turn over several steps without additional cost.
A \textit{fly-direct} action resets the commanded heading to the destination
bearing, eliminating accumulated drift in a single step.
This structure encodes the intended resolution pattern: issue one turn,
hold while the aircraft manoeuvres, then fly direct once the conflict clears.

Second, the reward signal is composed entirely of penalty terms.
The conflict penalty fires whenever a predicted separation violation is detected
($d_\text{CPA} < d_\text{sep}$) and scales with both the imminence and the
predicted miss distance, providing a graduated signal that reflects the
full geometry of the encounter.
...
Eight discrete actions are available.
Six \textit{turn instructions} set the commanded heading as a one-shot
...
Two additional actions require no heading computation.
\textit{Hold} (action~3) re-issues $\psi_\text{cmd}$ unchanged, maintaining
the current deviation without additional cost; it is the only truly free action.
\textit{Fly direct} (action~7) sets $\psi_\text{cmd} \leftarrow \psi_\text{dest}(f)$,
returning the aircraft immediately to its planned track.

The intended resolution pattern is: issue one turn instruction to create
separation, hold for the steps required to execute the turn and let the
conflict resolve, then fly direct to eliminate accumulated drift.
Workload cost is charged once per turn instruction.
All aircraft operate at a fixed cruise speed of $M = 0.78$.
...
$+2$ & $\hat{t}_\text{CPA}$        & $\operatorname{clip}(t_\text{CPA}/T_\text{warn},\,0,\,1)$  & $[0,1]$ \\
$+3$ & $\tilde{x}^\text{CPA}_{fj}$ & lateral CPA offset $/ \, d_\text{sep}$                    & $[-1,1]$ \\
$+4$ & $\tilde{y}^\text{CPA}_{fj}$ & forward CPA offset $/ \, d_\text{sep}$                    & $[-1,1]$ \\
\bottomrule
\end{tabular}
\end{table}
...
the reward function and is placed before the CPA coordinates to act as a
gate: a value of 1 signals that no conflict is predicted, making the
subsequent CPA features uninformative (both zero).
...
at most $(1 - \cos 45^\circ)/2 \approx 0.15$ at the largest deviation.
...
The hold action (action~2) is the only truly free action ($c = 0$).
For the fly-direct action (action~5) the cost scales with the commanded
...
$w_\text{drift}$    & Per-step deviation; accumulates during hold, motivates fly-direct        & $0.50$ \\
