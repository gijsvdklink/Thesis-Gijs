...
Observation space (24 floats, ego-centric from focus aircraft):
  ...
  [3]   direct_mode   1 if in fly-direct pursuit mode else 0    {0, 1}
  [4:24] 4 intruders x 5 (urgency-desc, dist-asc), local polar:
         ...
         empty/diverging slot: (1, 0, 1, 0, 1)   # cpa_r=1 (clean miss) at the safe end
...
Action space (Discrete 8) -- ATC-styled hybrid, consecutive:
  ...
  7  FLY DIRECT       psi_cmd = psi_dest    (small cost fly_direct_cost)  enters fly-direct mode
...
CONFIG = {
    ...
    'w_drift':               1.00,            # drift penalty at 90 deg. Piecewise-linear in |hdg err|:
                                               # 0 deg -> 0, knee (35 deg) -> drift_knee_pen, 90 deg ->
                                               # w_drift, continuing at the same slope beyond. Linear =>
                                               # an on-route aircraft costs ~0 (finite slope at 0).
    'drift_knee_deg':        35.0,            # knee angle of the piecewise-linear drift penalty
    'drift_knee_pen':        0.50,            # penalty magnitude at the knee (35 deg)
    'w_work':                1.50,            # charged per turn instruction (scales with |delta|); hold free
    'fly_direct_cost':       0.10,            # small fixed cost per fly-direct instruction (action 7)
    'seed':                  None,
}
...
class AirspaceEnv(gym.Env):
    ...
    def _compute_reward(self, acting_cs, action_idx):
        ...
        if acting_cs and acting_cs in self._destination_ll:
            ...
            if idx >= 0:
                ...
                # Piecewise-linear drift penalty in the heading-error angle:
                # 0 deg -> 0, knee (35 deg) -> drift_knee_pen, 90 deg -> w_drift,
                # continuing at the same slope beyond 90 deg. Being linear, an
                # on-route aircraft (angle ~ 0) costs ~0 with no special-casing,
                # while small deviations are already penalised on the first segment.
                a    = abs(hdg_err)                       # |heading error|, deg in [0, 180]
                knee = CONFIG['drift_knee_deg']
                pk   = CONFIG['drift_knee_pen']
                if a <= knee:
                    drift_pen = pk * (a / knee)
                else:
                    drift_pen = pk + (CONFIG['w_drift'] - pk) * (a - knee) / (90.0 - knee)
                r_drift = -drift_pen

        # Workload: one-time cost per instruction. Turns scale with |delta|; hold is
        # free; fly-direct carries a small fixed cost so issuing it is a real (if
        # cheap) instruction -- nudging the agent to issue it once and then hold
        # (direct mode persists) rather than re-spamming it every step.
        r_work = 0.0
        if acting_cs:
            if action_idx == 7:
                r_work = -CONFIG['fly_direct_cost']
            else:
    ...
    def _get_observation(self):
        ...
        # Direct-mode flag: exposes the (otherwise hidden) fly-direct pursuit state.
        # It changes what HOLD does -- in direct mode HOLD keeps tracking the
        # waypoint (free, stays on route); out of it HOLD holds the last commanded
        # heading (can drift). The agent cannot infer this from sin/cos(dpsi) alone
        # (both read ~0 in direct mode AND when coincidentally on-route), so it is
        # provided explicitly.
        direct_flag = 1.0 if self._direct_mode.get(cs, False) else 0.0

        obs = [
            math.sin(math.radians(hdg_err)),   # sin/cos encoding avoids +-180 discontinuity
            math.cos(math.radians(hdg_err)),
            turn_progress,
            direct_flag,                       # 1 = in fly-direct pursuit mode, else 0
        ]
            ...
            if dist_nm < sep:
                ...
                if 0 < tcpa <= t_warn:
                    ...
                    if cpa_east**2 + cpa_north**2 < sep**2:
                        ...
                        tcpa_n, cpa_r_n, cpa_theta_n = 1.0, 1.0, 0.0
                else:
                    tcpa_n, cpa_r_n, cpa_theta_n = 1.0, 1.0, 0.0
        ...
        for slot_k in range(N_NBR):
            if slot_k < len(selected):
                ...
                obs += [1.0, 0.0, 1.0, 0.0, 1.0]   # empty slot sentinel (cpa_r=1: clean miss)
