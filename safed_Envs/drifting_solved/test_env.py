...
Reward is purely negative (no positive components):
  ...
  -w_drift    x ((1-cos(psi_dest-psi_cmd))/2)                medium: commanded heading deviation from route,
                                                              linear -- 30/45/60 deg penalties stay close in
                                                              magnitude; even mild drift is costly enough to
                                                              make flying back worthwhile
  -w_work     x act_cost                                    small:  instruction workload
    turn 0/6 (+-60 deg): cost 2.0    turn 1/5 (+-45 deg): cost 1.5   turn 2/4 (+-30 deg): cost 1.0
    hold (3):             cost 0.0   fly-direct (7):                cost 0.0 (free)

Observation space (24 floats, ego-centric from focus aircraft):
  [0]   sin(delta_psi_dest)   heading error to destination   [-1, 1]
  [1]   cos(delta_psi_dest)                                   [-1, 1]
  [2]   turn_progress  (cmd_hdg - actual) / 180              [-1, 1]
  [3]   route_conflict  conflict score if flying direct now   [0,  1]
  [4:24] 4 intruders x 5 (urgency-desc, dist-asc), local polar:
...
Action space (Discrete 8) -- ATC-styled hybrid, consecutive:
  ...
  7  FLY DIRECT       psi_cmd = psi_dest         (free)   enters fly-direct mode
...
Fly-direct is free, to encourage returning to track as soon as it's safe to do so.
Fly-direct is a persistent mode: once issued, the commanded heading is re-aimed at the
destination every step (so the aircraft converges on the waypoint rather than holding a
stale one-shot bearing). It stays active through hold and is cancelled by any manual turn.
...
CONFIG = {
    ...
    'drift_resolved_frac':   0.05,            # drift fraction below this counts as "on-track"
                                               # (~26 deg); used to hold focus after a correction
    ...
    'w_drift':               0.863,           # drift penalty = w_drift * drift_frac**drift_exp;
                                               # concave (exp<1) shaping fit so 30 deg -> 0.50/step and
                                               # 90 deg -> 0.75/step -- small deviations punished hard
    'drift_exp':             0.202,            # see fit above (anchors 30 deg=0.5, 90 deg=0.75)
    'w_work':                1.50,            # charged once per turn instruction; hold and direct are free
    'seed':                  None,
}
...
OBS_DIM = 4 + N_NBR * 5       # 4 ownship + 20 intruder = 24
...
# 2x(30 deg) = 2.0 = 1x(60 deg).  Index 7 (fly-direct) is free (see _reward()).
ACT_COST = [2.0, 1.5, 1.0, 0.0, 1.0, 1.5, 2.0, None]
...
class AirspaceEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # re-aim all fly-direct aircraft at their destinations before propagating
    ...
    def _select_focus_aircraft(self):
        ...
        drift_thr   = CONFIG['drift_resolved_frac']
        for i, cs in enumerate(active):
            # steps since this aircraft last had a conflict (resets on urgency)
            if pair_max[i] > 0:
                self._steps_since_urgency[cs] = 0
            else:
                self._steps_since_urgency[cs] = self._steps_since_urgency.get(cs, clear_steps) + 1
            # steps since this aircraft was last meaningfully drifting (resets while off-route)
            if self._drift_frac(cs) > drift_thr:
                self._steps_since_drift[cs] = 0
            else:
                self._steps_since_drift[cs] = self._steps_since_drift.get(cs, clear_steps) + 1
        ...
        if self._focus_cs in active and best_cs != self._focus_cs:
            ...
            # Drift hold: while the whole sector is conflict-free, stay on the focus
            # aircraft until its drift has stayed resolved for clear_steps. This keeps
            # focus through the correction AND for a few low-cost steps afterwards, so
            # the agent actually experiences the reward of getting back on route rather
            # than losing focus the instant it does. Mirrors focus_resolved for conflicts.
            drift_hold     = (pair_max.max() == 0 and
                              self._steps_since_drift.get(self._focus_cs, clear_steps) < clear_steps)
            if (focus_pair_max > 0 or not focus_resolved or drift_hold) and not emergency:
    ...
    def _drift_fallback(self, active):
        ...
        Each aircraft is scored as drift x clearance, where clearance is
        1 - conflict_score: the inverse of its worst predicted dcpa/tcpa
        conflict score against any other aircraft (see _conflict_score).
        Drifted aircraft with the most dcpa/tcpa clearance are prioritised --
        those furthest (in miss-distance and/or time) from any predicted
        near-miss can turn back to their waypoint without creating a new
        conflict. A hysteresis margin prevents rapid focus switching.
        """
        best_cs     = None
        best_score  = -1.0
        focus_score = 0.0
        ...
        for cs in sorted(active):
            ...
            if idx < 0 or cs not in self._destination_ll:
                continue
            drift     = self._drift_frac(cs)
            clearance = 1.0 - self._conflict_score(cs)
            score     = drift * clearance
    ...
    def _compute_reward(self, acting_cs, action_idx):
        ...
        if acting_cs and acting_cs in self._destination_ll:
            ...
            if idx >= 0:
                ...
                # Concave shaping (exponent < 1) so even small deviations are
                # penalised hard: fit so 30 deg -> 0.5/step and 90 deg -> 0.75/step.
                drift_frac = (1.0 - math.cos(math.radians(hdg_err))) / 2.0
                r_drift    = -CONFIG['w_drift'] * drift_frac ** CONFIG['drift_exp']

        # Workload: one-time cost per instruction (turns only; hold and fly-direct are free)
        r_work = 0.0
        if acting_cs and action_idx != 7:   # guard: ACT_COST[7] is None (fly-direct is free)
    ...
    def _apply_action(self, cs, action_idx):
        ...
        if action_idx in TURN_DELTAS:
            ...
            # fly direct: enter direct mode -- the heading is re-aimed at the
            # destination every step (see _update_direct_headings) so the aircraft
            # converges on the waypoint instead of holding a stale, one-shot bearing.
    ...
    def _update_direct_headings(self):
        """Re-aim every fly-direct aircraft at its destination.

        FLY DIRECT only sets the bearing once; while the aircraft turns onto it
        it slides laterally off the straight line to the waypoint, and with LNAV
        off that cross-track offset is never recovered. Re-issuing HDG toward the
        current bearing each step turns it into a pursuit that converges on the
        waypoint. Stays active through HOLD; cleared by any manual turn.
        """
    ...
    def _get_observation(self):
        ...
        # Conflict score the ownship WOULD face if it flew direct to its
        # destination right now. The per-intruder features below are computed for
        # the current (possibly drifted) heading, so they read "all clear" while
        # coasting away -- they cannot tell the agent whether turning back to route
        # is safe. This channel supplies exactly that missing information.
        route_conflict = self._conflict_score(cs, dest_bearing)

        obs = [
            math.sin(math.radians(hdg_err)),   # sin/cos encoding avoids +-180 discontinuity
            math.cos(math.radians(hdg_err)),
            turn_progress,
            route_conflict,                    # [0, 1]: 0 = clear to fly direct
        ]
    ...
    def _process_exits(self):
        for cs in self._find_exited():
            ...
            self._steps_since_drift.pop(cs, None)
    ...
    def _spawn_aircraft(self, slot, ac):
        ...
        self._steps_since_drift[cs]   = CONFIG['focus_clear_steps']
        self._slots[slot]             = cs
