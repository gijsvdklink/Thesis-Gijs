...
\draw[->] (dly.north) -- (6.5,1.0)
          -- node[above,font=\scriptsize]{$\Delta\psi_\text{iss},\;r_\text{pend}$}
          (0,1.0) -- (agent.north);
\end{tikzpicture}

\caption{Per-step information flow. \textbf{(a)}~Immediate compliance: action takes effect within the same step, standard 24-feature MDP loop. \textbf{(b)}~With delay (dashed block — hidden from the agent): the issued heading queues until $t_\text{el}\ge d$; two extra signals feed directly back to the agent.}
\label{fig:flows}
\end{figure}
