...
The experimental environment augments the v4 observation vector from 25 to 27 features by appending two signals derived from the delay block:
...
$25$ & $\Delta\psi_\text{iss}$ & pending turn angle (issued $-$ effective heading) & $[-\pi,\pi]$\\
$26$ & $b_\text{exec}$         & 1 if instruction executed this step, else 0       & $\{0,1\}$\\
\bottomrule
\end{tabular}
\caption{Observation vector for the experimental environment ($o_k\in\mathbb{R}^{27}$). Features 0--24 are identical to v4; features 25--26 expose the pending-instruction state to the agent.}
\end{table}
...
Additional features
  & ---
  & $\Delta\psi_\text{iss}$,\; $b_\text{exec}$\\[3pt]
Reward trajectory
