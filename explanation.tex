...
\draw[->] (sim.south) -- (8.5,-1.8)
          -- node[below,font=\scriptsize]{$o_k$ (24 features),\quad$r_k$}
          (0,-1.8) -- (agent.south);
\end{tikzpicture}
...
The experimental environment augments the v4 observation vector from 24 to 26 features by appending two signals derived from the delay block:
...
$0$--$23$ & --- & see v4 specification & ---\\[3pt]
\multicolumn{4}{l}{\emph{New: pending-instruction state}}\\
$24$ & $\Delta\psi_\text{iss}$ & pending turn angle (issued $-$ effective heading) & $[-\pi,\pi]$\\
$25$ & $b_\text{exec}$         & 1 if instruction executed this step, else 0       & $\{0,1\}$\\
\bottomrule
\end{tabular}
\caption{Observation vector for the experimental environment ($o_k\in\mathbb{R}^{26}$). Features 0--23 are identical to v4; features 24--25 expose the pending-instruction state to the agent.}
\end{table}
