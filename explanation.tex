...
\draw[->] (dly.north) -- (6.5,1.0)
          -- node[above,font=\scriptsize]{$\Delta\psi_\text{iss},\;b_\text{exec}$}
          (0,1.0) -- (agent.north);
\end{tikzpicture}

\caption{Per-step information flow. \textbf{(a)}~Immediate compliance: action takes effect within the same step, standard 25-feature MDP loop. \textbf{(b)}~With delay (dashed block — hidden from the agent): the issued heading queues until $t_\text{el}\ge d$; two extra signals feed directly back to the agent.}
\label{fig:flows}
\end{figure}
...
The experimental environment augments the v4 observation vector from 25 to 28 features by appending three signals derived from the delay block:
...
\multicolumn{4}{l}{\emph{New: pending-instruction state}}\\
$25$ & $\Delta\psi_\text{iss}$ & uncommitted turn angle: issued $-$ effective & $[-\pi,\pi]$\\
$26$ & $\Delta v_\text{iss}$   & uncommitted speed change: issued $-$ effective, $/\,v_\text{nom}$ & ${\sim}[-1,1]$\\
$27$ & $b_\text{exec}$         & 1 if instruction executed this step, else 0 & $\{0,1\}$\\
\bottomrule
\end{tabular}
\caption{Observation vector for the experimental environment ($o_k\in\mathbb{R}^{28}$). Features 0--24 are identical to v4; features 25--27 expose the pending-instruction state to the agent.}
\end{table}

\noindent$\Delta\psi_\text{iss}=0$ and $b_\text{exec}=0$ indicate no outstanding instruction. $b_\text{exec}=1$ signals that the pending instruction executed this step; the agent can infer the aircraft has begun the manoeuvre.
...
Additional features
  & ---
  & $\Delta\psi_\text{iss}$,\; $\Delta v_\text{iss}$,\; $b_\text{exec}$\\[3pt]
Reward trajectory
