...
Ego-centric to focus $f$ (forward axis $=\psi_\text{act}$). All features are normalised
\emph{in-env} to $[-1,1]/[0,1]$ by fixed physical ranges, so training uses
\texttt{norm\_obs=False}. Here $\Delta\psi=\psi_\text{dest}-\psi_\text{cmd}$,
$v_\text{nom}=v/3600$\,NM/s, $d_\text{warn}=T_\text{warn}v_\text{nom}=37.5$\,NM.
...
$3$ & $p_\text{turn}$ & $\mathrm{wrap}(\psi_\text{cmd}-\psi_\text{act})/180^\circ$ (turn-execution lag) & $[-1,1]$\\
\multicolumn{4}{l}{\emph{Each intruder $j$ (slot $m$, indices $4{+}5m$ to $8{+}5m$)}}\\
$+0$ & $\rho$ & $d_{fj}/d_\text{warn}$ (range to intruder) & $[0,1]$\\
$+1$ & $\theta$ & ego bearing to $j$, $/\pi$ ($0=$ ahead) & $[-1,1]$\\
$+2$ & $\psi$ & intruder heading relative to ownship, $\mathrm{wrap}(\psi_j-\psi_\text{act})/180^\circ$ & $[-1,1]$\\
...
\texttt{norm\_obs=False} (observations are normalised within the environment),
\texttt{norm\_reward=True}.
