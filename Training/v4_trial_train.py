"""
PPO training for v4_trial (LiDAR-observation single-ownship environment).

Run:
    python -m Training.v4_trial_train
    python -m Training.v4_trial_train --multi   # 3 seeds in parallel

Monitor:
    tensorboard --logdir Runs_saved/v4_trial
"""
...
RUNS_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'Runs_saved', 'v4_trial')
)
...
def train(seed):
    run_name = f"v4_trial_9act_obs21_seed{seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
