"""
PPO training for v3_dalmau_dense (2-3 aircraft, high-density circular sector).

Run:
    python -m Training.v3_ATC_dalmau_dense
    python -m Training.v3_ATC_dalmau_dense --multi   # 3 seeds in parallel

Monitor:
    tensorboard --logdir Runs_saved/v3_dalmau_dense
"""
...
from Environments.v3_dalmau_dense import AirspaceEnv

# ── Settings ──────────────────────────────────────────────────────────────────

N_ENVS           = 24    # tuned for a 24-core server
TOTAL_TIMESTEPS  = 100_000_000
CHECKPOINT_EVERY = 300_000
N_RUNS           = 3

RUNS_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'Runs_saved', 'v3_dalmau_dense')
)

PPO_KWARGS = dict(
    learning_rate = 3e-4,
    n_steps       = 256,    # rollout = n_steps*N_ENVS = 6,144 (with N_ENVS=24)
                             # -- updates and reward/tensorboard dumps happen
                             # once per rollout, so this sets their frequency
    batch_size    = 1024,   # was 64: with a large N_ENVS the rollout is large,
                             # so batch_size=64 meant thousands of tiny
                             # minibatches x n_epochs, mostly Python/Adam
                             # overhead. 1024 -> 6 minibatches x 10 epochs = 60
                             # steps: far less wall-clock per rollout, and
                             # bigger batches give lower-variance gradients
                             # which tends to help PPO stability too.
    n_epochs      = 10,
    gamma         = 0.99,
    gae_lambda    = 0.95,
    clip_range    = 0.2,
