"""
PPO training for v3_env — single-agent ATCO conflict resolution.

The agent plays one ATCO controlling the whole sector: at each step it
issues a heading instruction to the most urgent aircraft.
...
Monitor:
    python -m tensorboard.main --logdir Runs_saved/bs/non_delay/v3
"""
...
from Environments.v3_env import AirspaceEnv
...
RUNS_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'Runs_saved', 'bs', 'non_delay', 'v3')
)

PPO_KWARGS = dict(
    learning_rate = 3e-4,
    n_steps       = 1024,          # > half a typical episode (~1200 steps)
    batch_size    = 256,
    n_epochs      = 10,
    gamma         = 0.99,
    gae_lambda    = 0.95,
...
def train(seed):
    ...
    env   = VecMonitor(make_vec_env(AirspaceEnv, n_envs=1))
    model = PPO(
        policy          = 'MlpPolicy',
        env             = env,
        seed            = seed,
    ...
    callbacks = CallbackList([
        ...
        CheckpointCallback(save_path=ckpt_dir, name_prefix='bs_v3_atco', seed=seed),
    ])
