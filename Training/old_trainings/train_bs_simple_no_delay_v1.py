"""
PPO training on AirspaceEnvV2 — MARL shared policy.

Each run creates a timestamped folder under Runs_saved/ containing:
    Runs_saved/bs/non_delay/simple/YYYYMMDD_HHMMSS_bs_simple_no_delay_v1/
        tensorboard/      ← TensorBoard logs
        checkpoints/      ← model snapshots every 50k steps
        model.zip         ← final saved model  (written on clean exit)

Stop training at any time with Ctrl+C.
The latest checkpoint in checkpoints/ is always safe to load.

Monitor a run:
    python -m tensorboard.main --logdir "Runs_saved/bs/non_delay/simple/YYYYMMDD_.../tensorboard"
...
from bs_simple_no_delay_v1 import MARLSingleAgentWrapper
...
def make_run_dir():
    ...
    name = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_bs_simple_no_delay_v1"
...
def main():
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        save_path   = ckpt_dir,
        name_prefix = 'bs_simple_no_delay_v1',
    )
    ...
    model.learn(
        ...
        tb_log_name         = 'bs_simple_no_delay_v1',
